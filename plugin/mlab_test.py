...
logging.basicConfig(format='%(relativeCreated)d - %(threadName)s - %(message)s',
...
class FakeValues(object):
  ...
  """
    ...
    def dispatch(self):
        ...
        keys.append(
            getattr(self, 'plugin_instance', None))  # may be unassigned.
        ...
        if metric_key in self._VALUE_MAP:  # pragma: no cover.
            ...
            raise ValueError('Dispatching duplicate metric key: %s' %
                             metric_key)
...
class MlabCollectdPlugin_VsysFrontendWithoutBackendTests(unittest.TestCase):
    ...
    def testunit_init_WHEN_no_target_RAISES_VsysCreateException(self):
        self.assertRaises(mlab.VsysCreateException, mlab.VsysFrontend,
                          'no_target')
...
class MlabCollectdPlugin_VsysFrontendTests(unittest.TestCase):
    ...
    def testunit_sendrecv_WHEN_shutdown_writer_RAISES_VsysException(self):
        ...
        self.assertRaises(mlab.VsysException, vsys.sendrecv, 'shutdown_writer',
                          1)
        vsys.close()
...
class MlabCollectdPlugin_UnitTests(unittest.TestCase):
    ...
    def testunit_plugin_configure(self):
        ...
        root_config = MockCfg('', (), (MockCfg('ExcludeSlice', ['pl_default']),
                                       MockCfg('ExcludeSlice', ['pl_netflow']),
                                       MockCfg('ExcludeSlice', ['pl_sfacm']),))
        expected_value = {'pl_default': True,
                          'pl_netflow': True,
                          'pl_sfacm': True}
...
class MlabCollectdPlugin_MetricTests(unittest.TestCase):
    ...
    @mock.patch('mlab.read_vsys_data')
    def testunit_report_meta_metrics(self, mock_read_vsys_data,
                                     mock_get_self_stats):
        ...
        self.assertEqual(
            metrics.get('fake.host/meta/collectd/process_memory/rss'),
            [2902900])
        self.assertEqual(
    ...
    def testunit_report_quota_for_vserver(self):
        ...
        self.assertEqual(
            metrics.get('fake.host/storage/vs_quota_bytes/quota'),
            expected_result)
    ...
    def testunit_report_threads_for_vserver(self):
        ...
        mlab.report_threads_for_vserver('fake.host', test_entry_path,
                                        fake_uptime)
        metrics = FakeValues.get_metrics()
        ...
        self.assertEqual(metrics.get('fake.host/context/vs_uptime'), [948500.0])
    ...
    def testunit_report_limits_for_vserver(self):
        ...
        self.assertEqual(metrics.get('fake.host/memory/vs_memory/vml'), [0.0])
        self.assertEqual(
    ...
    def testunit_report_cpuavg_for_system(self):
        ...
        self.assertEqual(metrics.get('fake.host/cpu_total/cpu/nice'), [87438.0])
...
class MlabCollectdPlugin_IntegrationTests(unittest.TestCase):
    ...
    def testintegration_plugin_read(self):
        ...
        self.assertEqual(
            metrics.get(
                'utility.mlab.fake.host/context/vs_threads_basic/other'), [19])
        self.assertEqual(metrics.get('fake.host/cpu_cores/gauge'), [2])
        self.assertEqual(
            metrics.get('fake.host/cpu_total/cpu/user'), [3147080.0])
        self.assertEqual(
