...
def fake_vc_get_dlimit(unused_c_filename, unused_c_xid, unused_c_flags,
                       c_limit):
...
class MlabVsResourceBackendTests(unittest.TestCase):
    ...
    def testunit_vc_get_dlimit_WHEN_vserver_lib_not_found_RAISES_LibVserverError(
        ...
        self.assertRaises(vs_resource_backend.LibVserverError,
                          vs_resource_backend.vc_get_dlimit, '/vservers', 515)
    ...
    @mock.patch('vs_resource_backend.pwd.getpwuid')
    def testunit_get_xid_names_WHEN_getpwuid_RAISES_KeyError(self,
                                                             mock_getpwuid):
        vs_resource_backend._VS_PREFIX_DIR = self._testdata_dir
    ...
    def testunit_handle_message_WHEN_type_IS_backend_stats(
        ...
        expected_value = (
            '{"message_type": "backend_stats", "version": 1, "data":'
    ...
    def testunit_handle_message_WHEN_type_IS_vs_xid_dlimits(
        ...
        mock_time.time.return_value = 10
        mock_get_xid_dlimits.return_value = {
            '515': [386460, 10000000, 9533, -1, 2]
        }
        expected_value = (
            '{"message_type": "vs_xid_dlimits", "version": 1, "data":'
    ...
    @mock.patch('vs_resource_backend.time')
    def testunit_handle_message_WHEN_type_IS_vs_xid_names(self, mock_time,
                                                          mock_get_xid_names):
        mock_time.time.return_value = 10
        mock_get_xid_names.return_value = {'515': 'mlab_utility'}
        expected_value = (
            '{"message_type": "vs_xid_names", "version": 1, "data":'
